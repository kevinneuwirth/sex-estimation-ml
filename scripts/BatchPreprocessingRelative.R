...
library(Morpho) # needed for cSize()
...
preprocessing = function(input){
  ...
  centroidtable[] <- lapply(centroidtable, as.numeric)
  centroid <- cSize(centroidtable)
  ...
  # convert absolute values into values relative to centroid size
  dist4 <- as.data.frame(t(dist4[,1:ncol(dist4)]/centroid))
  ...
  # convert absolute values into values relative to centroid size
  newcombntri <- as.data.frame(newcombntri[,1:ncol(newcombntri)]/centroid)
  ...
  databasepathandname <- paste0(databasepath,"\\","databaserelative.csv")
